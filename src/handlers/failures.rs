...
PO - Payout, where the insurer paid the depositors directly and placed the assets in a liquidating receivership. Note: Includes transactions where the FDIC established a Deposit Insurance National Bank to facilitate the payout process. This field can be used for search and filtering."#)]
    ...
    #[schemars(description = r#"Charter Class (Search-Eligible) - The FDIC assigns classification codes indicating an institution's charter type (commercial bank, savings bank, or savings association), its chartering agent (state or federal government), its Federal Reserve membership status (member or nonmember), and its primary federal regulator (state-chartered institutions are subject to both federal and state supervision). These codes are:
...
SB - State charter savings bank supervised by the FDIC. This field can be used for search and filtering."#)]
    ...
    #[schemars(description = r#"Date of Resolution - Date of Resolution. Usually the same as the fail date, except with banks with Open Bank Assistance."#)]
    pub resdate: Option<String>,

    #[schemars(description = r#"Resolution (Search-Eligible) - The given institution has failure stature or it can be assistance has been provided by FDIC in merging with other institution. This field can be used for search and filtering."#)]
    pub restype: Option<String>,

    #[schemars(description = r#"Total Deposits - Total including demand deposits, money market deposits, other savings deposits, time deposits and deposits in foreign offices as of the last Call Report or Thrift Financial Report filed by the institution prior to the effective date. Note this does not necessarily reflect total deposits on the last report filed because in some cases reports were filed after the effective date."#)]
    pub qbfdep: Option<f32>,

    #[schemars(description = r#"Total Assets - The Total assets owned by the institution including cash, loans, securities, bank premises and other assets as of the last Call Report or Thrift Financial Report filed by the institution prior to the effective date. Note this does not necessarily reflect total assets on the last report filed because in some cases reports were filed after the effective date. This total does not include off-balance-sheet accounts."#)]
    pub qbfasset: Option<f32>,

    #[schemars(description = r#"Estimated Loss - The estimated loss is the difference between the amount disbursed from the Deposit Insurance Fund (DIF) to cover obligations to insured depositors and the amount estimated to be ultimately recovered from the liquidation of the receivership estate. Estimated losses reflect unpaid principal amounts deemed unrecoverable and do not reflect interest that may be due on the DIF's administrative or subrogated claims should its principal be repaid in full.
Notes:
...
Also, for more detail regarding resolution transactions and the FDIC's receivership activities, see Managing the Crisis: The FDIC and RTC Experience, a historical study prepared by the FDIC's Division of Resolutions and Receiverships. Copies are available from the FDIC's Public Information Center."#)]
    ...
    #[schemars(description = r#"State (Search-Eligible) - Two-character alphanumeric code for US state or Territory This field can be used for search and filtering."#)]
...
impl IntoContents for FailuresResponse {
    fn into_contents(self) -> Vec<Content> {
        ...
        vec![Content::json(self).expect("Failed to serialize FailuresResponse to Content")]
    }
}
...
Responses:
    ...
    504: Gateway Timeout
Tag: Failures"#]
pub async fn failures_handler(config: &FdicApiConfig, params: &FailuresParameters) -> Result<CallToolResult, rmcp::Error> {
